...
export class ItemGenerator {
    ...
    private parseCustomWeaponSets() {
        ...
        for (const [
            ...
            const rarityColorHex = this.paintKitsRaritiesColorHex[paintKit.className] as CS2RarityColorValues
            ...
            for (const component of Object.entries(component_list)) {
                ...
                const component_loc = [`${paintKit?.nameToken}`, ' | ', `#${component[1]}`];
                const image_inventory = image_dir + "/" + component[1];
                this.addTranslation(id, "name", "#CSGO_Type_WeaponComponent", " | ", ...component_loc);
                this.addTranslation(idBlueprint, "name", "#CSGO_Type_Blueprint", " | ", ...component_loc);
                this.addItem({
                    base,
                    baseId,
                    id,
